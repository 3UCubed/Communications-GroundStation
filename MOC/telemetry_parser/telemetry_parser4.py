...
class TelemetryFile:
    def __init__(self, fname: str):
        ...
        # Setup logging to log errors into a text file
...
# MODIFIED Code for Error Log Generation
    def parse_file(self) -> None:
        fhdr = TelemetryFileHdr()

        f = open(self.fname, "rb") #original code
        fhdr.parse(f.read(TelemetryFileHdr.HDR_SIZE))

        print(fhdr) #orig code
        ...
        while byte:
            ...
            if byte[0] != 0:
                ...
                except Exception as exc: #if parsing fails, it should print an error the error then log the error!
                    print(f'Oops: Could not parse record -> {exc}')
                    self.logger.error(f"Error parsing message in file {self.fname}: {exc}")

                if not msg.is_crc_valid: #if crc check fails, then increment the invalid message counter
                    self.invalid_msg_cnt += 1

                if first_frame: #if first frame, mark the first frame as processed
                    prev_rollling_cntr = msg.rolling_cntr
                    first_frame = False
                else: #if the rolling counter does not increment, then log a dropped frame error
                    if ((prev_rollling_cntr + 1) % TelemetryMsg.MAX_TLM_ROLL_FRAME_CNT) != msg.rolling_cntr:
                        # Log dropped frame details
                        self._log_dropped_frame(f, current_pos)

                #Update the previous rolling counter to the current message's rolling counter
                prev_rollling_cntr = msg.rolling_cntr 

                #Append the parse message to the list of messages
                self.msglist.append(msg)

                #clear the message data list in preparation for the next message
                msgdata = []
        ...
        #printing the summary of the parsing process
        print(f'{len(self.msglist)} messages parsed | invalid count: {self.invalid_msg_cnt}')


    #function to log the details of a dropped frame
    def _log_dropped_frame(self, file_handler, byte_position):
        """Logs the dropped frame and the exact byte(s) from the .TLM file causing the drop."""
        frame_error_msg = (
            "\n...\n:exclamation_mark: [bold red]dropped frames[/bold red]\n...\n"
            f"Error occurred in file: {self.fname}\n"
            f"Function: {inspect.currentframe().f_code.co_name}\n"
            f"Line of Code: {inspect.currentframe().f_lineno}\n"
        )
        print(frame_error_msg) #print the error msg

        # Log the error and the line of code to the text file
        dropped_frame_msg = (
            f"Dropped frame detected in file {self.fname}: "
            f"Line {inspect.currentframe().f_lineno}, "
            f"Function {inspect.currentframe().f_code.co_name}\n"
            f"File: {__file__}\n"
        )

        # Move to the byte position where the drop occurred and read the context
        file_handler.seek(byte_position)
        file_content = file_handler.read(50)  # Read 50 bytes from the drop point as an example

        dropped_frame_msg += f"Context from the file (around drop point):\n{file_content}\n"

        # Print out the line of code for each dropped frame
        print(f"Dropped Frame Error: {dropped_frame_msg}")

        # Log the error into the file
        self.logger.error(dropped_frame_msg)
...
if __name__ == "__main__":
    ...
    for file in tlm_file_list:
        try:
            ...
            print(f'Oops: {exc}')
            tlm_file.logger.error(f"Error processing file {file}: {exc}")
        except KeyboardInterrupt:
            print("Program interrupted")
            # Log the interruption event
            if hasattr(tlm_file, 'logger'):
                tlm_file.logger.error("Program interrupted during file processing.")