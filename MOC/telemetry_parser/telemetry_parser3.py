...
class TelemetryFile:
    ...
    def parse_file(self) -> None:
        ...
        msgdata = []
        msg_idx = 0
        prev_rollling_cntr = None  # Initialize to None before the loop
        first_frame = True  # Use a flag to handle the first frame separately
        MIN_MESSAGE_SIZE = 1
        while byte:
            current_pos = f.tell()

            # Collect the byte in msgdata
            if byte[0] != 0:
            ...
            try:
                # Ensure enough data is available to parse
                if len(msgdata) < MIN_MESSAGE_SIZE:  # Define MIN_MESSAGE_SIZE
                    print(f"Skipping incomplete message at byte position {current_pos}, size: {len(msgdata)}")
                    msgdata = []  # Reset msgdata and continue
                    continue

                    msg.parse(cobs.decode(msgdata))  # Try parsing the message
            except Exception as exc:
                print(f'Oops: Could not parse record at byte position {current_pos} -> {exc}')
                print(f"Data around failure: {msgdata[-10:]}")  # Show context around failure
                msgdata = []  # Reset msgdata
                continue

            if not msg.is_crc_valid:
                self.invalid_msg_cnt += 1

            if prev_rollling_cntr and ((prev_rollling_cntr + 1) % TelemetryMsg.MAX_TLM_ROLL_FRAME_CNT) != msg.rolling_cntr:
                  print(f"\nDropped frame detected! Expected: {prev_rollling_cntr + 1}, Found: {msg.rolling_cntr}")

            prev_rollling_cntr = msg.rolling_cntr  # Update for next iteration

            self.msglist.append(msg)
            msgdata = []  # Reset msgdata for the next message
...
# from your_module import TelemetryFile, CSVFiles
...
if __name__ == "__main__":
    root_dir = os.path.dirname(__file__)
    tlm_dir = os.path.join(root_dir, "tlm_files")
   
    if not os.path.exists(tlm_dir):
        print(f"Directory {tlm_dir} does not exist.")
    else:
        tlm_file_list = glob.glob(os.path.join(tlm_dir, "*.TLM"))
       
        # Using ThreadPoolExecutor for I/O bound tasks or ProcessPoolExecutor for CPU bound
        with concurrent.futures.ThreadPoolExecutor() as executor:
            executor.map(process_file, tlm_file_list)